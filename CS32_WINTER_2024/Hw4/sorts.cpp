...
{
    // : You implement this.  Using the same criteria as compareSensor,
...
void sortUsingPtrs(vector<Sensor>& sensors, bool comp(const Sensor*, const Sensor*))
{
    ...
    for (int unsorted = 1; unsorted < pointers.size(); unsorted++)
    {
        Sensor* nextItem = pointers[unsorted];
        int loc = unsorted;
        while (loc > 0  &&  !comp(pointers[loc-1], nextItem))
        {
            pointers[loc] = pointers[loc-1];
            loc--;
        }
        pointers[loc] = nextItem;
    }
