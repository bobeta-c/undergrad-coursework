...
\solution{
    ...
    \] 
    and moving terms we now try to prove
    \[
...
\solution{
    \begin{enumerate}
        ...
        \item using the approximation we get
            \begin{align*}
                \frac{ (n!)^{3}}{(3n)!} &\approx \frac{(cn^{n+\frac{1}{2}}e^{-n})^{3}}{c(3n)^{3n+\frac{1}{2}}e^{-3n}}\\
...
\solution{
    ...
    We know that the original series converges if $|z| < R$ so we need
     \[
    |z_0 + (z-z_0)| < R \impliedby |z-z_0| < R - |z_0|
    \] 
...
\solution{
    ...
    but we have
    \[
        \frac{(m+n-1)!}{n!} = (m+n-1)(m+n-2)\dots(n+1) = n^{m-1} + O(n^{m-2})
    \] 
...
\solution{
    ...
    \lim_{x\to 0^{-}}\frac{f^{(n)}(x)}{x} = \lim_{x\to 0^{-}}f^{(n)} = 0
    \] 
    and so using an equivalent definition of the derivative we get
    \[
    f^{(n)}(0) = \lim_{h\to 0} \frac{f^{(n-1)}(-h) + f^{(n)}(h)}{2h} = 0
    \] 
    for all $n$.
    ...
    \] 
    then by theorem $2.6$ since  $f$ can be interpreted as a power series on $ \mathbb{C}$ we have that the derivates
    \[
