...
\title{\color{sectioncolor}W5 CS M146}
\author{}
\date{}
\maketitle
...
x \rightarrow \phi(x)
.\] 
can be more expressive but might have higher dimension.
we restrict $\phi $ to be derived from a kernel. Design $\phi(x)$ such that
\[
\langle \phi(x), \phi(u) \rangle
.\] 
can be computed as efficiently as 
\[
\langle x , u \rangle
.\] 
and expresss our learning algorithm such that it only requires computing
 \[
\langle \phi(x), \phi(u) \rangle
.\] 
